...
IDE가 자동으로 요청을 스킬에 매칭시킵니다. 스킬을 수동으로 선택할 필요 없습니다. 필요한 스킬만 컨텍스트에 로드됩니다.
...
| db-agent | 데이터베이스, 스키마, ERD, 마이그레이션 | 스키마 설계, 쿼리 튜닝 |
| mobile-agent | 모바일 앱, iOS/Android | Flutter 화면, 상태 관리 |
| brainstorm | 아이디에이션, 컨셉 탐색 | 설계 문서 |
| dev-workflow | CI/CD, git hooks, 모노레포 설정 | 워크플로우 설정, 자동화 |
| tf-infra-agent | Terraform, 클라우드 인프라 | IaC 모듈, 상태 관리 |
| translator | 번역, 다국어 콘텐츠 | 톤을 유지한 번역문 |
