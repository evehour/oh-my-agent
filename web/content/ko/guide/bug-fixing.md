...
워크플로우가 수정을 제안하고 적용한 후 같은 에러가 재발하면, 문제가 초기 진단보다 깊습니다. 이를 지원하는 워크플로우(ultrawork, orchestrate, coordinate)에서 **Exploration Loop**를 트리거합니다:
...
프론트엔드의 에러가 백엔드 변경에 의해 발생하고, 그것이 데이터베이스 스키마 마이그레이션에 의해 발생한 경우. 근본 원인이 도메인 경계를 넘을 때 관련 도메인 에이전트를 포함하도록 `/coordinate` 또는 `/orchestrate`로 에스컬레이션합니다.
