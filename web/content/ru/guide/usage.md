...
> Не знаете, с чего начать? Введите `/coordinate` и опишите, что хотите построить.
...
1. Определение ключевых слов видит мультидоменную задачу → предлагает `/coordinate`
...
| `/plan` | PM-декомпозиция задач → `.agents/plan.json` | Перед началом сложной фичи |
| `/exec-plan` | Выполнить существующий план пошагово | После `/plan` |
| `/coordinate` | Пошаговая мультидоменная координация | Фичи, охватывающие несколько агентов |
...
| `/commit` | Conventional commit с анализом type/scope | Фиксация изменений |
...
|-------|----------------|----------|
| oma-pm | «запланируй это», «декомпозируй» | `.agents/plan.json` |
...
| oma-commit | Git-коммиты | Conventional commits |
...
6. **Начинайте с `/coordinate`** — когда не уверены, какой workflow использовать
...
Для интеграции в существующие проекты смотрите [Руководство по Интеграции](./integration).
