...
**Шаги:** Подготовка -> Анализ объёма (оценка сложности: Simple/Medium/Complex) -> Создание плана выполнения (markdown в `docs/exec-plans/active/`) -> Определение API-контрактов (если кросс-доменное взаимодействие) -> Ревью с пользователем -> Выполнение (передача в `/orchestrate` или `/coordinate`) -> Завершение (перемещение в `completed/`).
...
Требуют явной `/command`: `/commit`, `/tools`, `/stack-set`, `/exec-plan`.
