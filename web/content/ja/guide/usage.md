...
| brainstorm | アイデア創出、コンセプト探索 | 設計ドキュメント |
| db-agent | データベース、スキーマ、ERD、マイグレーション | スキーマ設計、クエリチューニング |
| dev-workflow | CI/CD、git hooks、モノレポ設定 | ワークフロー設定、自動化 |
| tf-infra-agent | Terraform、クラウドインフラ | IaC モジュール、ステート管理 |
| translator | 翻訳、多言語コンテンツ | トーンを維持した翻訳文 |
