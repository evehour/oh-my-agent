...
**実行：** インライン。`/orchestrate`や`/work`で使用。出力：`.agents/results/plan-{sessionId}.json`。
...
**ステップ：** 準備 -> スコープ分析（複雑度の評価：Simple/Medium/Complex）-> 実行プラン作成（`docs/exec-plans/active/`にMarkdown）-> APIコントラクト定義（境界をまたぐ場合）-> ユーザーレビュー -> 実行（`/orchestrate`または`/work`にハンドオフ）-> 完了（`completed/`に移動）。
...
自動検出から除外（明示的`/command`が必要）：`/commit`、`/tools`、`/stack-set`、`/exec-plan`
