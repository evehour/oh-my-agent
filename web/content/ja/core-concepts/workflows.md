...
**ステップ：** 準備 -> スコープ分析（複雑度の評価：Simple/Medium/Complex）-> 実行プラン作成（`docs/exec-plans/active/`にMarkdown）-> APIコントラクト定義（境界をまたぐ場合）-> ユーザーレビュー -> 実行（`/orchestrate`または`/coordinate`にハンドオフ）-> 完了（`completed/`に移動）。
